...
test_url2=https://wit3.fbk.eu/archive/2017-01-ted-test//texts/$tgtlng/en/$tgtlng-en.tgz
wget -c $train_url -O train-en-$tgtlng.tgz
wget -c $test_url  -O test-en-$tgtlng.tgz
wget -c $test_url2 -O test-$tgtlng-en.tgz
if [ ! -d en-$tgtlng ]
then
    tar -xvzf train-en-$tgtlng.tgz
    tar -xvzf test-en-$tgtlng.tgz
    tar -xvzf test-$tgtlng-en.tgz
fi
if [ ! -f $src_train_raw ]
then
...
then
    ...
    cp $tgtlng-en/IWSLT17.TED.tst2017.$tgtlng-$srclng.$tgtlng.xml $tgt_test_raw
fi
...
TGT_TEST_PREPROCESSING="$TGT_TEST_PREPROCESSING | sed -e \"s/\’/\'/g\" | $REPLACE_UNICODE_PUNCT | $NORM_PUNC -l $tgtlng | $REM_NON_PRINT_CHAR | $TOKENIZER -l $tgtlng -no-escape -threads $N_THREADS"
fi
if [ ! -f $src_train_tok ]
then
...
then
    ...
    $FASTBPE applybpe $tgt_test_bpe $tgt_test_tok $bpecodes.$tgtlng
fi
